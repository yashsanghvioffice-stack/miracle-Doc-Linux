...
#
#  What it manages:
#    /opt/miracle-router/            -- Flask app + venv (service-owned)
...
# ─── 1. Flask app directory ─────────────────────────────────────
log "[1] /opt/miracle-router/  (Flask app + venv)"
...
# All directories inside: 755
# venv contents are handled separately (Python pip creates them with right perms,
# but ownership might be wrong if installed via sudo)
...
log "[2] /etc/miracle-registry/  (SQLite DB)"
...
# ─── 4. Nginx config ────────────────────────────────────────────
log "[4] /etc/nginx/sites-available/  (gateway config)"
...
# ─── 5. Systemd unit (contains API key -- lock down) ────────────
log "[5] /etc/systemd/system/miracle-router.service"

fix_path "/etc/systemd/system/miracle-router.service" "root:root" "640" "f"

# ─── 6. Log files ───────────────────────────────────────────────
log "[6] /var/log/miracle-*.log"
...
log "[7] /opt/miracle-backups/  (DB backups)"
...
# ─── 8. Git repo (production checkout) ──────────────────────────
log "[8] /opt/miracle-repo/  (git checkout)"
